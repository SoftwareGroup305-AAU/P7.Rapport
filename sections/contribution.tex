...
\textbf{Selectors.}
Selector nodes extract specific values from asset or indicator outputs, optionally applying a time offset (e.g., previous bar\todo{omformuler}). They provide a unified mechanism for selecting fields such as \textit{Close}, \textit{upper} (Bollinger upper band), or any indicator output. Selectors prepare data for condition nodes by ensuring that each operand refers to the correct value and temporal index.
...
We developed a dedicated machine learning service that encapsulates the LSTM model described in Section \ref{sec:method}. This service abstracts the complexities of TensorFlow and provides a high-level interface for the strategy builder.

The implementation provides an automated pipeline consisting of three stages:
\begin{enumerate}
    \item \textbf{Data Ingestion \& Transformation:} The service automatically fetches raw price data from the ClickHouse database, computes logarithmic returns, and applies Min-Max scaling. This ensures that the model always receives stationary, normalized data without manual intervention.
    \item \textbf{Model Lifecycle Management:} The system handles the initialization, training, and serialization of models. Trained models are saved alongside their metadata (hyperparameters, scaling factors, and training timestamps) to ensure reproducibility.
    \item \textbf{Batched Inference:} To support efficient backtesting, the service implements a vectorized inference engine. Instead of predicting one time-step at a time, the system constructs batches of sliding windows and processes them in parallel on the GPU, if available, significantly reducing the time required to evaluate strategies over long historical periods.
\end{enumerate}

This modular implementation allows the machine learning component to be updated or swapped (e.g., changing the architecture or hyperparameters) without affecting the rest of the strategy framework.
...
\textbf{Bitcoin Results: The High-Volatility Case}
