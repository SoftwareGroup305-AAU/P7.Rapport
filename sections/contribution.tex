...
The AI-Only strategies achieved the highest average return (614.75\%) with a peak configuration reaching 6145.69\%, accompanied by a win rate of 42.1\%. However, these results must be interpreted with caution due to severe training bias. The LSTM model was trained on only 2023 data, a single year dominated by a sustained bull market. This limited training window biased the model toward predicting upward price movements, which proved correct during the majority of the test period (2024--2025 bull continuation). Consequently, the high win rate and returns reflect regime-specific overfitting rather than robust predictive capability. The elevated average drawdown of 27.09\% and low profit factor of 1.84 further suggest that the model lacks adaptive risk management during market reversals.
...
Table~\ref{tab:jpm_results} presents the aggregated performance metrics for JPMorgan Chase. The results contrast sharply with the Bitcoin experiment, demonstrating significantly higher win rates and lower drawdowns across all categories.
...
This contrasts sharply with Bitcoin, where the AI-Only approach appeared to dominate due to the confluence of (1) training bias aligned with a sustained bull market, (2) test period regime similarity to training period, and (3) high volatility that amplified the returns of any bullish-biased strategy. In that context, filters primarily served to reduce risk rather than enhance returns, because they constrained the very bias that was (coincidentally) profitable.
