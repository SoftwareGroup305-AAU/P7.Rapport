...
\subsection{Data Acquisition and Storage}\todo{commented out section}
% CHANGED FROM THIS %%
...
%% TO THIS %%
Data Providers\todo{paragraph subtitle inline} Working with financial market data requires reliable providers that offer high-quality datasets. While many options exist, the primary candidates were Yahoo Finance, EODHD, and Massive (formerly Polygon.io) for stock data, along with Coinbase and Binance for cryptocurrency data.

For cryptocurrency, the most consistent dataset—\todo{dash removal}specifically for Bitcoin (BTC)\todo{dash removal}—was sourced from Coinbase. Regarding stock data, Massive was selected as it provides access to consistent historical data across the American stock market. Because both providers are free to use and offer consistent data quality, they were selected for the cryptocurrency and stock markets, respectively \todo{source for providers}.

Data Representation\todo{paragraph subtitle inline} Since financial time series data is typically represented in the Open-High-Low-Close (OHLC)\todo{abbreviation consistency} format (often extending to OHLCV), and the aim of the project is to make a general-purpose time series prediction framework, this format must be generalized. To represent financial market data as general-purpose time series, each OHLCV entry is split into multiple series, where each series contains only a single vector of data points representing each metric.

Each series represents\todo{contains} the metadata concerning a set of data points, such as the asset, metric, temporal coverage, and interval resolution. Each data point is represented as a value, a timestamp, and a corresponding series identifier. This relationship allows for a uniform way to represent time series regardless of domain. All data points are single values at a specific timestamp, and the interpretation of the value depends entirely on the metadata of the related series.
...
The design of the strategy components was based on widely adopted statistical models and data-manipulation techniques. These building blocks were integrated into a modular framework in which strategies are represented as DAGs. Each node performs a well-defined transformation\todo{transformation / action}, such as fetching data, computing indicators, evaluating conditions, or generating signals. Nodes are connected via typed inputs and outputs to produce end-to-end trading\todo{forecasting logic?} logic.

\begin{itemize}
    \item \textbf{Assets:} Asset nodes retrieve historical price data from the Data Warehouse\todo{general api} API for a given trading pair\todo{explain trading pair} and timeframe. In normal operation, an asset node provides only a single configured price metric (Open, High, Low, or Close). Complete OHLCV\todo{abbreviation consistency} candlestick\todo{explain candlestick} data is not sourced from the asset node itself but from a separate global setting responsible for supplying full market data required for accurate financial backtesting. Asset nodes therefore function as lightweight, metric-specific data feeds for all downstream components.
    ...
    \item \textbf{Selectors:} Selector nodes extract specific values from asset or indicator outputs, optionally applying a time offset (e.g., previous bar\todo{omformuler}). They provide a unified mechanism for selecting fields such as \textit{Close}, \textit{upper} (Bollinger upper band)\todo{explain bollinger bands}, or any indicator output. Selectors prepare data for condition nodes by ensuring that each operand refers to the correct value and temporal index.
...
Each node receives a univariate price series from a preceding selector, which extracts a specific field (e.g., close price) from an asset's candlestick\todo{financial term} data. Using this input, the AINode applies a pre-trained LSTM time-series model configured through parameters such as the trading pair\todo{financial term}, timeframe, model name, and prediction horizon.

The node operates in a walk-forward manner. A fixed start date defines the beginning of the training window, while the end of the window advances with each bar in the dataset\todo{is the start date fixed?}. At every bar\todo{financial term} $t$, the model loads its windowed training data and produces a vector\todo{tensor} of forecasts for the next $N$ periods (e.g., $\hat{p}_{t+1}, \hat{p}_{t+2}, \ldots, \hat{p}_{t+N}$). These predictions are stored as a nested structure, one forecast vector\todo{tensor} per historical bar\todo{financial term}, and can be specified using selector nodes.
...
To capture non-linear temporal dependencies in financial time series, the system employs a Long Short-Term Memory (LSTM)\todo{abbreviation consistency} network. LSTMs are a specialized variant of Recurrent Neural Networks (RNNs)\todo{abbreviation consistency} designed to address the vanishing gradient problem inherent in standard RNNs\todo{inherent to many nns}, which hampers the ability to learn long-term dependencies \cite{Hochreiter1997}.
...
\begin{figure}[H]\todo{hvem har dog brugt tid på dette?}\todo{kunne eventuelt sættes i bilag, hvis ikke det passer på siden}
\centering
...
\subsubsection{Strategy Archetypes} \todo{wierd afsnit indentation her}
\begin{enumerate}
