...
\textbf{Data Representation.} Since financial time series data is typically represented in the Open-High-Low-Close (OHLC)\todo{abbreviation consistency} format (often extending to OHLCV), and the aim of the project is to make a general-purpose time series prediction framework, this format must be generalized. To represent financial market data as general-purpose time series, each OHLCV entry is split into multiple series, where each series contains only a single vector of data points representing each metric.
